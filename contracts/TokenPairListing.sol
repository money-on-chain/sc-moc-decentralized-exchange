...
contract TokenPairListing is ConfigurableTick, EventfulTokenPairListing {
   ...
   */
    modifier isNewPairValid(address _baseToken, address _secondaryToken) {
        ...
        require(
            !validPair(_secondaryToken, _baseToken),
            "Existent inverse pair"
        );
  ...
  */
    ...
    function setTokenPairSmoothingFactor(
        ...
        require(
            _smoothingFactor <= PRECISION_SMOOTHING_FACTOR,
            "Smoothing factor should be in relation to 1"
        );
  ...
  */
        ...
        isNewPairValid(_baseToken, _secondaryToken)
    {
        ...
        tokenPairs[pairIndex] = MoCExchangeLib.Pair(
            MoCExchangeLib.Token(
                MoCExchangeLib.Data(0, 0, 0, 0, 0, true),
                IERC20(_baseToken)
            ),
            MoCExchangeLib.Token(
                MoCExchangeLib.Data(0, 0, 0, 0, 0, false),
            ...
            TickState.Data(
                SafeMath.add(block.number, tickConfig.minBlocksForTick),
                0,
                0,
                1
            ),
   ...
   */
        ...
        )
    {
        ...
        return (
            pair.tickState.number,
            pair.tickState.nextTickBlock,
            pair.tickState.lastTickBlock
        );
    }
  ...
  */
    function getTokenPair(address _baseToken, address _secondaryToken)
        internal
        view
        returns (MoCExchangeLib.Pair storage pair)
    {
  ...
  */
    function tokenPair(address _baseToken, address _secondaryToken)
        internal
        view
        returns (MoCExchangeLib.Pair storage)
    {
