...
contract MoCDecentralizedExchange is EventfulExchange, RestrictiveOrderListing, PartialExecution {
  ...
  */
  function matchOrders(address _baseToken, address _secondaryToken, uint256 steps)
  ...
  */
  function matchOrdersWithHints(address _baseToken, address _secondaryToken, uint256 steps, uint256[] calldata hintIds)
    external whenNotPaused {
  ...
  */
  function processExpired(
    address _baseToken, address _secondaryToken, bool _isBuy, uint256 _orderId, uint256 _previousOrderIdHint, uint256 _steps
  ) external whenNotPaused {
    ...
    while (currStep < _steps && toEvaluate.id != 0) {
      ...
      if (MoCExchangeLib.isExpired(toEvaluate, pair.tickState.number)) {
        ...
        MoCExchangeLib.processExpiredOrder(commissionManager, token, toEvaluate.id, toEvaluate.exchangeableAmount, toEvaluate.reservedCommission, toEvaluate.owner);
  ...
  */
  function getTokenPairStatus(address _baseToken, address _secondaryToken) external view
  returns(
    uint64 tickNumber,
    uint256 nextTickBlock,
  ...
  */
  ...
  /**
    ...
    @return lastSellMatchId Id of the last Sell order to match
  */
  function getEmergentPrice(address _baseToken, address _secondaryToken) public view
  ...
  */
  function addTokenPair(
    ...
    bytes32[] memory taskList = new bytes32[](uint(TaskTypes.length));
    taskList[uint(TaskTypes.SIMULATION)] = getTaskId(groupId, TaskTypes.SIMULATION);
    taskList[uint(TaskTypes.MATCHING)] = getTaskId(groupId, TaskTypes.MATCHING);
    taskList[uint(TaskTypes.MOVING_OUT_PENDINGS)] = getTaskId(groupId, TaskTypes.MOVING_OUT_PENDINGS);
    createTask(
      taskList[uint(TaskTypes.SIMULATION)],
      simulationStepFunction,
      onSimulationStart,
      onSimulationFinish);
    createTask(
      taskList[uint(TaskTypes.MATCHING)],
      matchOrdersStepFunction,
      onMatchOrdersStart,
      nullHookForTask);
    createTask(
      taskList[uint(TaskTypes.MOVING_OUT_PENDINGS)],
  ...
  */
  ...
  /**
    ...
    for the execution of a tick of a given pair
  */
  function simulationStepFunction(bytes32 _groupId, bytes32, uint256)
  ...
  */
   ...
   */
  function matchOrdersStepFunction(bytes32 _groupId, bytes32, uint256)
    private returns (bool) {
    MoCExchangeLib.Pair storage pair = getTokenPair(_groupId);
  ...
  */
  function movePendingOrdersStepFunction(bytes32 _groupId, bytes32, uint256)
    ...
    bool movedBuyOrder = MoCExchangeLib.movePendingOrderFrom(
      ...
      true);
    if (!movedBuyOrder) {
      MoCExchangeLib.movePendingOrderFrom(pair.secondaryToken, pair.pageMemory, address(pair.baseToken.token), address(pair.secondaryToken.token), false);
    }
