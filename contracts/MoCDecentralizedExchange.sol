    ...
    PartialExecution
{
  ...
  */
    function processExpired(
        ...
        MoCExchangeLib.Token storage token = _isBuy
            ? pair.baseToken
            : pair.secondaryToken;
        MoCExchangeLib.Order storage toEvaluate = _orderId == 0
            ? token.orderbook.first()
            : token.orderbook.get(_orderId);
  ...
  */
    ...
    @param _secondaryToken Address of the secondary token of the pair
  */
    function enableTokenPair(address _baseToken, address _secondaryToken)
        public
        whenTickIsNotRunning(_baseToken, _secondaryToken)
    {
    ...
    @param _secondaryToken Address of the secondary token of the pair
  */
    function tickIsRunning(address _baseToken, address _secondaryToken)
        public
        view
        returns (bool)
    {
  ...
  */
    function getTickStage(address _baseToken, address _secondaryToken)
        public
        view
        returns (MoCExchangeLib.TickStage stage)
    {
  ...
  */
    function onSimulationStart(bytes32 _groupId, bytes32) private {
        ...
        pair.pageMemory.lastBuyMatch = pair
            .baseToken
            .orderbook
            .getNextValidOrder(pair.tickState.number, 0);
        pair.pageMemory.lastSellMatch = pair
            .secondaryToken
            .orderbook
            .getNextValidOrder(pair.tickState.number, 0);
    }
  ...
  */
    function onSimulationFinish(bytes32 _groupId, bytes32) private {
        ...
        if (pair.pageMemory.matchesAmount > 0) {
            pair.pageMemory.emergentPrice = Math.average(
                pair.pageMemory.lastBuyMatch.price,
  ...
  */
        ...
        returns (bool shouldKeepGoing)
    {
        ...
        return
            pendingSellOrdersLength(pair) != 0 ||
            pendingBuyOrdersLength(pair) != 0;
    }
  ...
  */
   ...
   */
    ...
    @return Task id
   */
    function getTaskId(bytes32 _groupId, TaskTypes _taskType)
        private
        pure
        returns (bytes32)
    {
    ...
    @return Group id
   */
    function getGroupIdForPair(address _baseToken, address _secondaryToken)
        private
        pure
        returns (bytes32)
    {
