...
library MoCExchangeLib {
  ...
  */
  function removeOrder(Data storage self, Order storage _toRemove, uint256 _startFromId) public {
    if (isFirstOfOrderbook(self, _toRemove)) { // If first order, re-assing the linked list start to next
      ...
      if (isLastOfOrderbook(_toRemove)) { // If last Order, and not only, tails previous
        previousOrder.next = 0;
      } else {  // if in the middle, link prevoius to next
  ...
  */
   ...
   */
  function positionOrderAsPending(
    Data storage self,
    uint256 _orderId
  ) private
  {
  ...
  */
  ...
  function priceGoesBefore( Data storage self, uint256 _price, uint256 _existingPrice) private view returns(bool) {
    return self.orderDescending && (_price > _existingPrice) ||
  ...
  function getNextValidOrder(
    ...
    if (next.id == 0 || !isExpired(next, _tickNumber))
      return next;
    else
      return getNextValidOrder(_orderbook, _tickNumber, next.id);
  }
   ...
   */
  function getStatus(Pair storage _self) internal view
    returns(
      uint64 tickNumber,
  ...
  */
  function doInsertOrder(
    Pair storage _self, uint256 _id, uint256 _exchangeableAmount, uint256 _reservedCommission, uint256 _price, uint64 _lifespan, uint256 _previousOrderIdHint,
    address _sender, address _receiver, bool _isBuy
   ...
   */
  function convertToBase(uint256 _secondary, uint256 _price, uint _priceComparisonPrecision) internal pure returns (uint256) {
    return _secondary
      .mul(_price)
      .div(_priceComparisonPrecision);
  }
  ...
  function isValid(Pair storage _self) internal view returns (bool) {
    return address(_self.baseToken.token) != address(0) &&
  ...
  */
  function emitNewOrderEvent(
    uint256 _orderId, Pair storage _self, address _sender, uint256 _exchangeableAmount, uint256 _reservedCommission, uint256 _price, uint64 _expiresInTick, bool _isBuy
  ) private {
    emit NewOrderInserted(
      _orderId, _sender, address(_self.baseToken.token), address(_self.secondaryToken.token),
      _exchangeableAmount, _reservedCommission, _price, _expiresInTick, _isBuy
    );
  }
  ...
  */
  function getEmergentPrice(Pair storage _self) public view
  returns(uint256 emergentPrice, uint256 lastBuyMatchId, uint256 lastBuyMatchAmount, uint256 lastSellMatchId) {
    ...
    while (shouldMatchMemory(buy,sell)) {
      ...
      (uint256 limitingAmount, MatchType matchType) = compareIntents(
        buy.exchangeableAmount, buy.price, sell.exchangeableAmount, _self.priceComparisonPrecision
      );
  ...
  */
  function executeSellerMatch(
    ...
    // For Token transfer, we use SafeTransfer to protect loop against individual reverts
    if (!SafeTransfer.doTransfer(_pair.baseToken.token, _sell.owner, buyerSent))
      buyerSent = 0;
  ...
  function subtractAmount(
    MoCExchangeLib.Order storage _order,
    uint256 _sent
  ) private {
  ...
  function processExpiredOrder(
    ...
    (bool transferResult, uint256 returnedAmount, uint256 commission, uint256 returnedCommission) = refundOrder(
      _commissionManager, _token.token, _exchangeableAmount, _reservedCommission, _owner, true
    );
  ...
  */
  function getFirstForMatching(
    CommissionManager _commissionManager, Token storage _token, uint64 _tickNumber
  ...
  */
  ...
  function movePendingOrderFrom(
    ...
    if (_token.orderbook.amountOfPendingOrders == 0)
      return false;
    // pop from queue
