...
let testHelper;
describe('Deletion of Market Orders from the Pending Queue in the after match', function() {
  let dex;
  let base;
  let secondary;
  let pair;
  let assertTickStage;
  const DEFAULT_MULTIPLY_FACTOR = 1.5;
  ...
  contract(
    'SCENARIO: 2 pending sell marekt order inserted with decreasing multiplyFactor we move them in the order which they were inserted',
    function(accounts) {
      const [, , seller] = accounts;
      // eslint-disable-next-line mocha/no-sibling-hooks
      before(initContractsAndAllowance(accounts));
      givenTheTickIsRunning(accounts, function() {
        describe('AND there are two sell orders pending', function() {
          ...
          describe('WHEN calling matchOrders with just one step', function() {
            ...
            it(
              'THEN the first inserted pending market order was moved into the main orderbook',
    ...
    givenTheTickIsRunning(accounts, function() {
      describe('AND there are 2 pending buy orders and 2 pending sell orders', function() {
        ...
        describe('WHEN calling matchOrders with just one step', function() {
          ...
          it(
            'THEN the first pending buy market order was moved into the main orderbook',
            testBuyOrderAtIndex({
              expectedId: 5,
              expectedIndex: 0
            })
          ...
          it(
            'AND the other pending buy market order remains in the pending queue',
