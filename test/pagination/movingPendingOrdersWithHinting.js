...
let testHelper;
describe('Depletion of the Pending Queue in the after match using hints', function() {
  let dex;
  let base;
  let secondary;
  let baseAddress;
  ...
  contract(
    'SCENARIO: N pending order put at the same time, matched with hints consume less gas',
    function(accounts) {
      const [, , seller] = accounts;
      ...
      givenTheTickIsRunning(accounts, function() {
        describe('AND there are still sell orders pending', function() {
          ...
          describe('WHEN calling matchOrders with just the enough steps and without hints THEN the tx reverts', function() {
            before(function() {
              return expectRevert.unspecified(
                dex.matchOrders(...pair, amountOfOrders, { gas: 6e6 })
              );
            });
            describe('BUT WHEN calling matchOrders with just the enough steps and the necessary hints', function() {
              before(function() {
                /**
                 * The paginated function needs to run one step to enter the second group,
                 * move the existing pending order and finish the pagination
      ...
      givenTheTickIsRunning(accounts, function() {
        describe('AND there are still 3 sell order pending', function() {
          ...
          describe('WHEN calling matchOrders with just the enough steps and the hints calculated before inserting the last one', function() {
            ...
            it('AND the sell pending queue still has the last order', testPendingSellLength(1));
