...
if (!input.baseToken || !input.secondaryToken || !input.mocState)
  throw new Error(
    'Usage: node changePriceProviderMocBproUsd.js <baseToken> <secondaryToken> <mocState> [<network>]'
  );

const execute = async () => {
  const newProviderAddress = await deployMocBproUsdPriceProviderFallback(
    input.network,
    input.baseToken,
